...
foursquare.table = function() {
    var template = _.template(
        ...
        '<div class="venue">' +
            '<div class="location"><a href="#<%= id %>"><%= name %></a><br><br> ' +
            '<%= location.address %> ' +
                '<% if(location.crossStreet) { %><%= location.crossStreet %><%}%>' +
                '<% if(location.city !=="San Francisco") { %><br><em><%= location.city %></em><%}%>' +
            '</div><br><br>' +
            '<% if(contact.twitter) { %>' +
            '<div class="phone"><a href="http://twitter.com/<%= contact.twitter %>">@<%= contact.twitter %></a></div>' +
            '<%}%>' +
            '<% if(stats.checkinsCount) { %>' +
            '<div class="phone"><%= stats.checkinsCount %>+ Checkins</div>' +
            '<%}%>' +
        '</div>'
    );
